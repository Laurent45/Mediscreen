...
class NoteControllerTest {
    ...
    @Mock private INoteService noteServiceMock;
    @InjectMocks private NoteController noteControllerUT;

    @Test
    void whenGetNotes_thenReturnAllNotes() {
        List<Note> notes =
                Arrays.asList(
                        new Note("Dr james", 1L, "report", LocalDateTime.now()),
                        new Note("Dr Mike", 2L, "report report", LocalDateTime.now()));
    ...
    void getId_whenGetNoteById_thenReturnNote() {
        Note note = new Note("Dr james", 1L, "report", LocalDateTime.now());
    ...
    void getIdPatient_whenGetNotesByPatientId_thenReturnAllNotes() {
        List<Note> notes =
                Arrays.asList(
                        new Note("Dr james", 1L, "report", LocalDateTime.now()),
                        new Note("Dr Mike", 1L, "report report", LocalDateTime.now()));
